...
class ArucoMockPublisherNode(Node):
    ...
    def __init__(self):
        ...
        self.marker_img = np.zeros(
            (self.marker_size_px, self.marker_size_px), dtype=np.uint8
        )
        self.marker_img = cv2.aruco.drawMarker(
            aruco_dict, self.marker_id, self.marker_size_px, self.marker_img, 1
        )
        ...
        self.color_image = (
            np.ones((self.image_height, self.image_width, 3), dtype=np.uint8) * 255
        )  # white background
        ...
        self.color_image[
            y_offset : y_offset + self.marker_size_px,
            x_offset : x_offset + self.marker_size_px,
            0,
        ] = self.marker_img
        self.color_image[
            y_offset : y_offset + self.marker_size_px,
